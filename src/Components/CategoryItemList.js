        ...
        <View style={[styles.serviceContainer, { elevation: 10, }]}>
            ...
            <View style={{ width: '80%', paddingVertical: moderateScale(4), }}>
                ...
                <View style={{ flexDirection: 'row', justifyContent: "space-between" }}>
                    <Text style={[styles.serviceName, { marginTop: 0, textAlign: 'left', color: colors.green, textDecorationLine: 'line-through' }]} numberOfLines={1}>{service?.actual_price > 0 ? '₹' : null}{service?.actual_price > 0 ? service?.actual_price : ''}</Text>
...
const CategoryItemList = () => {
    ...
    const handleEndReached = async () => {
        await dispatch(setCurrentCategoryItemPage(categoryItemsCurrentPage + 1))
        await dispatch(GetSelectedCategoryItemsMethod(selectedCategory))
    }
