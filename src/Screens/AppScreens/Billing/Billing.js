...
const Billing = () => {
  ...
  const renderCategoryItemsItem = useCallback(({ item }) => (
    ...
    <View style={styles.categoryItemsItem}>
      ...
      <View style={{ width: '80%', justifyContent: 'space-between', flexDirection: 'row' }}>
        <Text style={[styles.itemPrice, { textDecorationLine: 'line-through' }]} numberOfLines={1}>{item.actual_price > 0 ? '₹' : ''}{item.actual_price > 0 ? item.actual_price : ''}</Text>
  ...
  // console.log("Billing", recommendedData[0])
  return (
