...
import { AddCategoryMethod, AddMasterCategoryMethod, AddSubCategoryMethod } from '../../../../config/userApiMethods';
...
const SubCategoryAdd = ({ route }) => {
  const { masterCategoryId } = route?.params
  ...
  const handleSubmit = async () => {
    ...
    const resp = await dispatch(AddSubCategoryMethod(inputs, masterCategoryId))
    console.log("SubCategoryAdd", resp)
    ...
    if (resp?.status === true) {
      var id = await resp?.data?.category_id
      console.log("<SubCategpryAdd>", resp, id)


      if (id && id?.toString().length > 0) {
        // console.log(id);

        const url = `${BASE_URL}category/save-image-by-category/${id}`
  ...
  return (
    <View style={{ backgroundColor: "white" }}>
      <HeaderComp
        screenName={'Add New Sub-Category'}
