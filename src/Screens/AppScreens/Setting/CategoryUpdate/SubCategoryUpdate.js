...
import { GetSubCategoryMethod, updateCategoryMethod } from '../../../../config/userApiMethods';
...
const SubCategoryUpdate = ({ route }) => {
    const { category, masterCategoryId } = route.params;
    ...
    const filteredCategory = subCategory?.filter(item => item?.id === category);
    const initialCategoryName = filteredCategory[0]?.category || '';


    // console.log(subCategory, "subCategory",masterCategoryId)
    ...
    const handleSubmit = async () => {
        ...
        if (resp?.status === true) {
            ...
            console.log("<SubCategory>", resp, categoryId)
            await dispatch(GetSubCategoryMethod(masterCategoryId))
    ...
    return (
        <View style={{ flex: 1, backgroundColor: '#FFF' }}>
            <HeaderComp
                screenName={'Update Sub-Category'}
