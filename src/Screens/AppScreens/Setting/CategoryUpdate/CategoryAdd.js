...
import { AddCategoryMethod, AddMasterCategoryMethod } from '../../../../config/userApiMethods';
...
const CategoryAdd = () => {
    ...
    const handleSubmit = async () => {
        if (!inputs.master_category_name) {
            showToast("Please Input category name")
            return;
        }
        console.log("handle", inputs);
        // Your logic for handling the submission
        const resp = await dispatch(AddMasterCategoryMethod(inputs))
        if (resp?.status === true) {
            var master_category_id = await resp?.data?.master_category_id
            console.log("<>", resp, master_category_id)

            if (master_category_id && master_category_id?.toString().length > 0) {
                console.log(master_category_id);
                // const url = `${BASE_URL}category/save-image-by-category/${master_category_id}`
                const url = `${BASE_URL}Master-category/save-image-by-master/${master_category_id}`
                imgUpload(url)
            } else {
        ...
        <View style={{ backgroundColor: "white" }}>
            ...
            <View style={{ height: '100%', marginTop: 32, gap: 32, alignItems: 'center', }}>
                ...
                <View style={{ width: '100%' }}>
                    ...
                    <TextInputCompo
                        onChangeText={(text) => handleOnChange(text, 'master_category_name')}
                        onFocus={() => setErrors({ ...errors, master_category_name: null })}
                        iconName="shape-plus"
                        placeholder="Enter Category Name"
                        // maxLength={10}
                        error={errors.master_category_name}
                    />

                    <ButtonCompo onPress={handleSubmit} title="Save" style={{}} />
                </View>
            </View>
        </View>
    );
