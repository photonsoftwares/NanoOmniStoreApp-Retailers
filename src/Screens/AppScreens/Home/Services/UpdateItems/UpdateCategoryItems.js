...
// import { CategoryItemUpdateMethod, GetCategoryItemMethod, GetCategoryMethod, } from '../../../../../config/userApiMethods';
...
//     const {  recommendedCurrentPage } = useSelector((state) => state?.recommendedReducer);
//     const { categoryItemsData,  } = useSelector((state) => state?.categoryItemsReducer)

//     const itemToUpdate = categoryItemsData.find((item) => item?.item_id === itemId);
//     // console.log("itemToUpdate", itemToUpdate.category)
...
// export default UpdateCategoryItems;
...
const UpdateCategoryItems = ({ route }) => {
    ...
    const itemToUpdate = subCategoryItems?.find((item) => item?.item_id == itemId);
    const _actual_price = itemToUpdate?.actual_price

    // console.log("UpdateCategoryItems", itemToUpdate?.actual_price, itemId, itemToUpdate?.actual_price || '',_actual_price)

    const [itemName, setItemName] = useState(itemToUpdate?.item_name || '');
    // const [actualPrice, setActualPrice] = useState(_actual_price);
    ...
    const handleUpdate = async () => {
        ...
        if (ItemUpdateMethod_resp?.status) {
            ...
            if (hasURI === true) {
                ...
                console.log("ImgUpdate", hasURI === true)
                ...
                >
                    <View style={styles.container}>
                        ...
                        <TextInput
                            ...
                            value={actualPrice}
                            // value={_actual_price}
                            // value={itemToUpdate?.actual_price}
                            onChangeText={(text) => setActualPrice(text)}
                            keyboardType="numeric"
                            placeholderTextColor="#666"
                            placeholder='Mrp'
