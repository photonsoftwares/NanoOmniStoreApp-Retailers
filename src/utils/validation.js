...
export const validateWalletBalanceForm = (inputs) => {
	let errors = {};
	const balance = /^[0-9]+$/;

	if (!inputs.balance) {
		errors.balance = 'Please input balance';
	} else if (inputs.balance == 0) {
		errors.balance = 'Min balance of ₹1';
	} else {
		errors.balance = null; // Reset the error if no error
	}
...
export const validateCoupanForm = (inputs) => {
	let errors = {};
	const balance = /^[0-9]+$/;

	if (!inputs.balance) {
		errors.balance = 'Please input coupan name';
	} else if (inputs.balance == 0) {
		errors.balance = 'Min balance of ₹1';
	} else {
		errors.balance = null; // Reset the error if no error
